...
//            MaxSize = new Vector2(64, 256), // This killing 32x48* sprites
    ...
    public void UpdateState(DetailExaminableEuiState state, IEntityManager entManager)
    {
        ...
        // SD-ERPStatus-Start
        if (state.ERPStatus == EnumERPStatus.FULL)
            PreviewERPStatus.FontColorOverride = Color.Green;
        else if (state.ERPStatus == EnumERPStatus.HALF)
            PreviewERPStatus.FontColorOverride = Color.Yellow;
        else
            PreviewERPStatus.FontColorOverride = Color.Red;

        PreviewERPStatus.Text = GetERPStatusName(state.ERPStatus);
    ...
    // SD-ERPStatus-start
    public string GetERPStatusName(EnumERPStatus status)
    {
        switch (status)
        {
    ...
    private bool PlayerBadge() // TODO: Something like donator, coder, owner badges
    {
        ...
        var grantedCKeys = new[] { "puroslavking", "sablerti" }; // /me показывает язык ящеру
